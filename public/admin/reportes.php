...
switch ($reporteActual) {
    ...
    case 'temas_criticos':
        ...
        $filtros_data['estados'] = ['PENDIENTE', 'EN_PROCESO', 'ATENDIDA', 'CERRADA'];
    ...
    case 'diagnosticos':
        ...
        foreach ($diagEntries as $d) {
            ...
            if (!empty($filtros['ciudad']) && (!isset($d['contact']['ciudad']) || stripos($d['contact']['ciudad'], $filtros['ciudad']) === false)) continue;
            if (!empty($filtros['perfil']) && ($d['perfil'] ?? '') !== $filtros['perfil']) continue;
            if (!empty($filtros['dificultad'])) {
                if (empty($d['dificultades']) || !in_array($filtros['dificultad'], $d['dificultades'], true)) continue;
            }
...
$total_resultados = $is_analitico
    ? array_sum(array_map('count', array_filter($analitico_data, 'is_array')))
    : count($resultados);
?>
<!DOCTYPE html>
<html lang="es">
...
>>>>>>> 0e14ba81545d64a74b8afec77948313128a00816
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form method="GET" class="row g-3">
                ...
                <?php if ($reporteActual == 'citas' || $reporteActual == 'capacidad_operativa' || $reporteActual == 'temas_criticos'): ?>
                    <div class="col-md-3">
                ...
                <?php if ($reporteActual == 'diagnosticos' || $reporteActual == 'usuario_riesgo'): ?>
                    <div class="col-md-3">
                        <label class="form-label">Fecha Inicio</label>
                        <input type="date" name="fecha_inicio" class="form-control" value="<?= htmlspecialchars($filtros['fecha_inicio']) ?>">
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="ciudad" class="form-select">
                            ...
                            <?php foreach ($filtros_data['cities'] ?? [] as $c): ?>
                                <option value="<?= htmlspecialchars($c) ?>" <?= ($filtros['ciudad'] === $c) ? 'selected' : '' ?>><?= htmlspecialchars($c) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="perfil" class="form-select">
                            ...
                            <?php foreach ($filtros_data['perfiles'] ?? [] as $p): ?>
                                <option value="<?= htmlspecialchars($p) ?>" <?= ($filtros['perfil'] === $p) ? 'selected' : '' ?>><?= htmlspecialchars($p) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="dificultad" class="form-select">
                            ...
                            <?php foreach ($filtros_data['dificultades'] ?? [] as $d): ?>
                                <option value="<?= htmlspecialchars($d) ?>" <?= ($filtros['dificultad'] === $d) ? 'selected' : '' ?>><?= htmlspecialchars($d) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                <?php endif; ?>
    ...
    <?php if (!empty($resultados) || $is_analitico): ?>
        <?php $export_query = http_build_query(array_merge($_GET, ['pdf' => 1])); ?>
        <?php if ($is_analitico): ?>
        ...
        <?php if ($is_analitico): ?>
            <?php foreach ($analitico_data as $seccion_nombre => $datos_seccion): ?>
                ...
                <?php elseif (is_array($datos_seccion) && array_key_exists(0, $datos_seccion) && is_array($datos_seccion[0])):
                ...
                <?php elseif (is_array($datos_seccion)): ?>
                    ...
                    <table class="table table-sm table-borderless w-50">
                        <tbody>
                            <tr>
                                ...
                                <td><?= htmlspecialchars((string)$datos_seccion) ?></td>
                            </tr>
                        </tbody>
                    </table>
                <?php endif; ?>
        ...
        <?php else: ?>
            ...
            <?php else: ?>
                <?php $keys = !empty($resultados) ? array_keys($resultados[0]) : []; ?>
                <table class="table table-bordered table-hover">
                    <thead>
                        <tr>
...
(function(){
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', function(){
            ...
            if (reporte === 'capacidad_operativa') {
    ...
    const modalEl = document.getElementById('diagDetailModal');
    if (modalEl) {
        const detailEl = document.getElementById('diagDetailHtml');
        const bsModal = new bootstrap.Modal(modalEl);
        document.querySelectorAll('.btn-view-detail').forEach(btn => {
            btn.addEventListener('click', function(){
                detailEl.innerHTML = this.getAttribute('data-detail') || 'Sin detalle disponible.';
                bsModal.show();
            });
        });
    }
